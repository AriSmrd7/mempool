...
const tokens = {
  ...
  purchaseAmount: process.env.PURCHASEAMOUNT || "0.007",
  pair: [
    "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c",
    options.token,
  ],
  GASLIMIT: process.env.GASLIMIT || "1200000",
...
if (!GLOBAL_CONFIG.WALLET) {
  throw(
      "The public address (WALLET) was not found in .env. Enter your public address in .env."
  );
}
...
const startConnection = () => {
  ...
  provider._websocket.on("open", () => {
    console.log(
      `Watching the txpool for events for token ${options.token}...`
    );
